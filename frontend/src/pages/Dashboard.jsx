...
const GLOBAL_STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .slide-in-right { animation: slideInRight 0.35s cubic-bezier(0.22,1,0.36,1) both; }
  ...
  .btn-icon-sm.focus:hover  { background: rgba(99,102,241,0.1); color: #6366f1; }
...
const COLUMNS = [
  {
    ...
    emptyIcon: (
      <svg style={{ width: 32, height: 32, opacity: 0.2 }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function Dashboard() {
  ...
  // Otomatik arşivlemenin bir kez çalışması için ref
  ...
  useEffect(() => {
    ...
    ]).then(([tasksRes, userRes]) => {
      setTasks(tasksRes.data)
      setUser(userRes.data.user)
    })
  }, [])

  // Görevler yüklendikten sonra süresi dolmuş olanları otomatik arşivle
  useEffect(() => {
    if (tasks.length === 0 || autoArchiveRan.current) return
    autoArchiveRan.current = true

    const expiredTasks = tasks.filter(
      t => t.status !== 'done' && isExpired(t.dueDate)
    )
    if (expiredTasks.length === 0) return

    // Her birini API'ye arşivle
    Promise.all(
      expiredTasks.map(t =>
        api.post(`/tasks/${t._id}/archive`, {
          reason: 'Süre doldu',
          archivedAt: new Date().toISOString(),
        })
      )
    ).then(() => {
      const expiredIds = new Set(expiredTasks.map(t => t._id))
      setTasks(prev => prev.filter(t => !expiredIds.has(t._id)))
      if (activeTask && expiredIds.has(activeTask._id)) setActiveTask(null)
    }).catch(() => {
      // API yoksa sadece state'den kaldır
      const expiredIds = new Set(expiredTasks.map(t => t._id))
      setTasks(prev => prev.filter(t => !expiredIds.has(t._id)))
      if (activeTask && expiredIds.has(activeTask._id)) setActiveTask(null)
    })
  ...
  const handleEditSave = async () => {
    ...
    if (activeTask?._id === editTask._id) setActiveTask(res.data)
    setEditTask(null)
  }
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setEditTask(null) }}>
          <div className="modal-box">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <button onClick={() => setEditTask(null)} className="btn-icon-sm delete" style={{ width: 32, height: 32, fontSize: 18, borderRadius: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <input className="field" placeholder="Görev başlığı" value={editTask.title} autoFocus
                onChange={e => setEditTask({ ...editTask, title: e.target.value })}
                onKeyDown={e => e.key === 'Enter' && handleEditSave()}
              />
              <textarea className="field" placeholder="Açıklama (opsiyonel)" value={editTask.description || ''}
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {user && (
          <div className="fade-up" style={{
            ...
            boxShadow: '0 20px 60px -15px rgba(99,102,241,0.45)', color: 'white',
          }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <button
                    className={`pomodoro-toggle${pomodoroOpen ? ' active' : ''}`}
                    onClick={() => setPomodoroOpen(p => !p)}
                  >
                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    {pomodoroOpen ? 'Pomodoro Kapat' : 'Pomodoro Aç'}
                  </button>
        ...
        {/* ── Main layout: Kanban + Pomodoro panel ── */}
        <div className="fade-up-2" style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>

          {/* ── Kanban Board ── */}
          <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16, alignItems: 'start', minWidth: 0 }}>
            {COLUMNS.map((col, colIdx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Tasks */}
                  <div className="col-scroll" style={{ padding: '12px', flex: 1, display: 'flex', flexDirection: 'column', gap: 8, minHeight: 80, overflowY: 'auto', maxHeight: 560 }}>
                    ...
                    {colTasks.map(task => {
                      ...
                      return (
                        <div
                          ...
                          onDragEnd={onDragEnd}
                          className={`task-card${isDragging ? ' dragging' : ''}${isPop ? ' card-pop' : ''}${isFocused ? ' active-task' : ''}`}
                          style={{
                            background: 'var(--bg)',
                            border: '1px solid var(--border)',
                            borderRadius: 12,
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6, flex: 1 }}>
                              ...
                              {isFocused && <span className="focus-badge">⏱ Odak</span>}
                            </div>
                            <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                              <button
                                className="btn-icon-sm focus"
                                title={isFocused ? 'Odaktan çıkar' : 'Pomodoro ile odaklan'}
                                onClick={() => handleFocusTask(task)}
                                style={{ color: isFocused ? '#6366f1' : undefined }}
                              >
                                <svg width="12" height="12" fill={isFocused ? '#6366f1' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                              </button>
          ...
          {/* ── Pomodoro Panel ── */}
          {pomodoroOpen && (
            <div className="pomodoro-panel slide-in-right">
              <PomodoroTimer
                activeTask={activeTask}
                onSessionEnd={(sessionType) => {
                  console.log('Seans bitti:', sessionType)
                }}
